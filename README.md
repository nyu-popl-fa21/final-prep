...
2. Implement a Scala function
   ...
   def countSat[A](p: A => Boolean)(l: List[A]): Int = l match {
     case Nil => 0
     case hd :: tl =>
       (if (p(hd)) 1 else 0) + countSat(p)(tl)
   }
   ...
   def countSat[A](p: A => Boolean)(l: List[A]): Int = {
     ...
     def cs(lp: List[A], acc: Int): Int = lp match {
        case Nil => acc
        case hd :: tl => cs(tl, (if (p(hd)) 1 else 0) + acc)
     }
     cs(l, 0)
   }
   ...
   def countSat[A](p: A => Boolean)(l: List[A]): Int =
     l.foldLeft(0){ (acc, hd) => (if (p(hd)) 1 else 0) + acc }
...
2. ```javascript
   ...
   From the definition of `f` in the const declaration we infer that `f` has
   the type `tf` defined by the equation:

   `tf = t4 => t5 => t4`

   From the call `swap(f)` we can now infer that the program is only well-typed if:

   `t1 = tf`

   Using the equations defining `t1` and `tf` we can infer that this is only true if
...
4. ```javascript
   ...
   program well-typed. If we consider subtyping, then any subtype of
   this type is also a valid annotation. For example, with subtyping
   we can also use the following type for `t1`:
...
2. A simpler and even more efficient solution is to make the function
   ...
   def fibTail(n: Int, i: Int, r: Int, s: Int): Int = {
     if (i == n) s else fibTail(n, i + 1, r + s, r)
   }
   ```
